...
// its in-memory-only persistence for the rest of the process. Starting the
// SecureStore round trip here, before config/firebase.ts's initializeAuth()
// call even runs (module evaluation order guarantees this module finishes
// first, since firebase.ts imports it), means the key is already resolved
// or in flight by the time that one-shot probe happens, instead of racing a
// cold SecureStore lookup against it. Errors are swallowed here (not left as
// an unhandled rejection) because getOrCreateEncryptionKey() is re-awaited by
// the real setItem/getItem calls below, which already handle failures.
